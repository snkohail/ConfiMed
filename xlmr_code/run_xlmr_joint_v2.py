...
def main():
    parser = argparse.ArgumentParser(description='Run XLM-R Joint Training Experiment V2 (Optimized)')
    parser.add_argument('--input_file', type=str, required=True, help='Path to the JSON dataset file')
    parser.add_argument('--output_dir', type=str, default='./results_xlmr_joint_v2', help='Directory to save results')
    ...
    en_col = 'english_email' if 'english_email' in df.columns else 'text_en'
    ar_col = 'arabic_email' if 'arabic_email' in df.columns else 'text_ar'
    
    print(f"Using text columns: {en_col} and {ar_col}")
    ...
    for fold_idx, (train_index, test_index) in enumerate(skf.split(df, df['label'])):
        ...
        # Combine English and Arabic for Training
        train_texts_en = df.iloc[train_index][en_col].tolist()
        train_texts_ar = df.iloc[train_index][ar_col].tolist()
        train_labels = df.iloc[train_index]['label'].tolist() * 2 
        train_texts = train_texts_en + train_texts_ar
        
        # SHUFFLE Training Data (Crucial for Joint Training)
        train_texts, train_labels = shuffle(train_texts, train_labels, random_state=42)
        
        # Combine English and Arabic for Testing
        test_texts_en = df.iloc[test_index][en_col].tolist()
        test_texts_ar = df.iloc[test_index][ar_col].tolist()
        test_labels = df.iloc[test_index]['label'].tolist() * 2
        test_texts = test_texts_en + test_texts_ar
    ...
    print("\n=== Final Optimized Joint Training Results ===")
