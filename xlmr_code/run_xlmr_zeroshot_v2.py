...
def main():
    parser = argparse.ArgumentParser(description='Run XLM-R Zero-Shot Experiment V2 (Optimized)')
    parser.add_argument('--input_file', type=str, required=True, help='Path to the JSON dataset file')
    parser.add_argument('--train_lang', type=str, required=True, choices=['en', 'ar'], help='Source language')
    parser.add_argument('--test_lang', type=str, required=True, choices=['en', 'ar'], help='Target language')
    parser.add_argument('--output_dir', type=str, default='./results_xlmr_zeroshot_v2', help='Directory to save results')
    ...
    lang_map = {
        'en': 'english_email' if 'english_email' in df.columns else 'text_en',
        'ar': 'arabic_email' if 'arabic_email' in df.columns else 'text_ar'
    }
    
    train_col = lang_map[args.train_lang]
    test_col = lang_map[args.test_lang]
    ...
    for fold_idx, (train_index, test_index) in enumerate(skf.split(df, df['label'])):
        ...
        train_texts = df.iloc[train_index][train_col].tolist()
        train_labels = df.iloc[train_index]['label'].tolist()
        
        # Test on Target Language
        test_texts = df.iloc[test_index][test_col].tolist()
    ...
    print("\n=== Final Optimized Zero-Shot Results ===")
