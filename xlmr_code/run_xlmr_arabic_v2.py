...
def main():
    parser = argparse.ArgumentParser(description='Run XLM-R Arabic Experiment V2 (Optimized)')
    parser.add_argument('--input_file', type=str, required=True, help='Path to the JSON dataset file')
    parser.add_argument('--output_dir', type=str, default='./results_xlmr_ar_v2', help='Directory to save results')
    ...
    # Check for Arabic text column
    if 'arabic_email' in df.columns:
        text_col = 'arabic_email'
    elif 'text_ar' in df.columns:
        text_col = 'text_ar'
    else:
        raise ValueError("Input JSON must contain 'arabic_email' or 'text_ar' field.")
